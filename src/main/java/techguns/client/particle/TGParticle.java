...
public class TGParticle extends Particle implements ITGParticle {
    ...
    public void renderParticle(VertexConsumerProvider.Immediate vertexConsumerProvider, Entity cameraEntity, float partialTickTime, float rotX, float rotZ, float rotYZ, float rotXY, float rotXZ, Matrix4f mat, Camera camera)
    {
		...
		buffer.vertex(mat, (float)p1.x + fPosX, (float)p1.y + fPosY, (float)p1.z + fPosZ).texture(ua, va).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(0, 240).next();//.normal(0.0f, 1.0f, 0.0f).endVertex();
		buffer.vertex(mat, (float)p2.x + fPosX, (float)p2.y + fPosY, (float)p2.z + fPosZ).texture(ub, vb).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(0, 240).next();//.normal(0.0f, 1.0f, 0.0f).endVertex();
		buffer.vertex(mat, (float)p3.x + fPosX, (float)p3.y + fPosY, (float)p3.z + fPosZ).texture(uc, vc).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(0, 240).next();//.normal(0.0f, 1.0f, 0.0f).endVertex();
		buffer.vertex(mat, (float)p4.x + fPosX, (float)p4.y + fPosY, (float)p4.z + fPosZ).texture(ud, vd).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(0, 240).next();//.normal(0.0f, 1.0f, 0.0f).endVertex();
